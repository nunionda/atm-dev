...
- **멀티 전략 엔진** — Momentum Swing (6-Phase) + SMC 4-Layer + Breakout-Retest + ES-F 인트라데이 스코어링
- **ES-F 인트라데이 스캘핑** — AMT 3-Stage Filter + Z-Score 통계 + Volume Profile 기반 데이트레이딩 (ES/MES)
- **S&P500 선물 스윙** — 4-Layer 스코어링 양방향(롱/숏) + 마진/서킷브레이커/롤오버 시뮬레이션
...
- **히스토리컬 백테스트** — 유니버스/선물/인트라데이 백테스트 + Monte Carlo 스트레스 테스트
- **웹 대시보드** — 12개 페이지 (차트 분석, 운영 모니터링, 선물 트레이딩, ESF 스캘핑, 옵션 계산기 등)
...
| API | `api/futures_routes.py` | S&P500 선물 분석/백테스트 |
| API | `api/esf_intraday_routes.py` | ES-F 인트라데이 분석/백테스트 |
...
| Strategy | `strategy/breakout_retest.py` | Breakout-Retest 돌파 후 리테스트 전략 |
| Strategy | `strategy/sp500_futures.py` | S&P500 선물 4-Layer 스윙 전략 |
| Strategy | `strategy/esf_intraday.py` | ES-F 인트라데이 AMT+Z-Score 전략 |
| Risk | `risk/risk_manager.py` | 리스크 게이트 (RG1-RG4) |
| Backtest | `backtest/historical_engine.py` | 히스토리컬 백테스터 |
| Backtest | `backtest/futures_backtester.py` | 선물 백테스터 (마진/CB/롤오버) |
| Backtest | `backtest/intraday_backtester.py` | 인트라데이 백테스터 (세션 기반) |
| Backtest | `backtest/metrics.py` | 성과 지표 (Sharpe, MDD, Win Rate, Monte Carlo) |
...
| `/futures-trading` | FuturesTrading | S&P500 선물 4-Layer 분석/백테스트 |
| `/esf-scalping` | ESFScalping | ES-F 인트라데이 스캘핑 (AMT+Z-Score+VP) |
...
프론트엔드와 백엔드를 **별도 터미널**에서 동시에 실행해야 합니다:
...
| Method | Path | 설명 |
|--------|------|------|
| GET | `/futures/tickers` | 지원 선물 티커 목록 (13종) |
| GET | `/futures/analyze/{ticker}` | 4-Layer 스코어링 분석 |
| GET | `/futures/signal/{ticker}` | 진입 시그널 생성 |
| POST | `/futures/backtest` | 선물 백테스트 실행 |
| GET | `/futures/contract-specs` | 계약 사양 + 세션 정보 |

### ES-F 인트라데이 (데이트레이딩)

| Method | Path | 설명 |
|--------|------|------|
| GET | `/esf/analyze/{ticker}` | AMT 상태 + 4-Layer 인트라데이 스코어 + VP |
| GET | `/esf/signal/{ticker}` | 인트라데이 시그널 (Grade A/B/C) |
| GET | `/esf/session-status` | 오늘 세션 상태 (트레이드, P&L, 연속 손절) |
| GET | `/esf/volume-profile/{ticker}` | Volume Profile (POC/VAH/VAL) |
| POST | `/esf/backtest` | 인트라데이 백테스트 (15m 바 기반) |
| GET | `/esf/backtest/status` | 진행 상태 |
| GET | `/esf/backtest/result` | 결과 (세션별 메트릭, Monte Carlo) |
...
| `breakout_retest` | BRT 돌파/리테스트 파라미터 |
| `sp500_futures` | 선물 4-Layer 스코어링, Z-Score, ATR exit, 마진 |
| `esf_intraday` | ES-F 인트라데이 AMT, VP, Grade 임계값, 세션 리스크 |
