...
# Phase 4.C 분할: 모델/상수/StrategyAllocator는 별도 모듈로 이관.
# 외부 import 호환을 위해 모두 re-export.
from .models import (  # noqa: F401
    SimSystemState, SimPosition, SimOrder, SimSignal,
    SimRiskMetrics, SimTradeRecord, SimEquityPoint, SimPerformanceSummary,
)
from .engine_constants import (  # noqa: F401
    WATCHLIST, OnEventType,
    REGIME_PARAMS, STOCK_REGIME_THRESHOLDS,
    _STOCK_REGIME_LEVEL,
    STOCK_REGIME_STRATEGY_AFFINITY, REGIME_EXIT_PARAMS,
    BASE_KELLY, REGIME_OVERRIDES,
    VIX_SIZING_SCALE,
...
# Phase 4.C-3: pure helpers (ADX, level clustering) → _helpers.py
from ._helpers import compute_adx as _compute_adx  # noqa: F401  (external compat)
from ._helpers import cluster_levels as _cluster_levels_fn  # used by SimulationEngine._cluster_levels staticmethod
...
class SimulationEngine:
    ...
    def _cluster_levels(levels: list, tolerance: float = 0.015) -> list:
        """Phase 4.C-3: 실제 구현은 _helpers.cluster_levels 로 이관. 호환 위해 staticmethod alias 유지."""
        return _cluster_levels_fn(levels, tolerance)
    ...
    def _check_exits_smc(self):
        ...
        for code, pos in self.positions.items():
            ...
            # ── Phase E: R-Multiple 분할 청산 (ES1 전, 비파괴적) ──
            _rm_exit = self._check_r_multiple_partial_exit(pos, current_price)
            ...
            elif atr_val and atr_val > 0 and not exit_reason:
    ...
    def _check_exits_mean_reversion(self):
        ...
        for code, pos in self.positions.items():
            ...
            # ATR SL (2일 쿨다운)
            elif not exit_reason and atr_val and atr_val > 0:
    ...
    def _execute_buy(
        ...
        # ── Phase E: R-Multiple 1R 단위 기록 ──
        # 1R = entry로부터 stop_loss까지의 거리. ES2A/ES2B 부분 청산의 기준점.
        # ATR 기반 stop이 좁으면 ATR 거리, -5% 캡에 걸리면 5% 거리 → 자동으로 적정값.
        r_unit_value = max(effective_price - stop_loss, effective_price * 0.02)  # 최소 2% guard

        # ── Phase F: Holding Tier 분류 ──
        # signal/trend/stage/regime 종합 점수로 LONG/MID/SHORT 분류
        # tier별로 _check_r_multiple_partial_exit에서 차등 적용
        holding_tier_value = self._classify_holding_tier(
        ...
        # tier별 누적 진입 카운터
        self._phase_stats.setdefault(f"tier_{holding_tier_value}_entries", 0)
        self._phase_stats[f"tier_{holding_tier_value}_entries"] += 1
        ...
        self.positions[signal.stock_code] = SimPosition(
            ...
            r_unit=round(r_unit_value, 4),  # Phase E: R-Multiple 분할 청산용 1R 단위
            holding_tier=holding_tier_value,  # Phase F: LONG/MID/SHORT 분류
        )
    ...
    def _classify_holding_tier(
        ...
        """Phase F: Entry 시점 holding tier 분류.

        4개 요소 가중 점수 (max 11점):
          - signal_strength (0-100): >=80→3, >=60→2, else→1
          - trend_strength (STRONG/MODERATE/WEAK): 3 / 2 / 1
          - trend_stage (EARLY/MID/LATE): 3 / 2 / 1
          - market_regime: STRONG_BULL/BULL→2, NEUTRAL→1, else→0

        분류:
          score >= 9 → LONG  (강한 추세, 큰 winner 추종)
          score >= 6 → MID   (표준, Phase E 기본)
          else       → SHORT (약한 시그널, 빠른 회수)
        """
        score = 0
        # 시그널 강도
    ...
    def _check_r_multiple_partial_exit(self, pos: SimPosition, current_price: float) -> Optional[tuple]:
        ...
        Tier별 차등 (pos.holding_tier 기반):
          SHORT: +0.8R 50% 청산 → +1.5R 50% 청산 (작게 빨리)
          MID  : +1.0R 33% 청산 → +2.0R 50% 청산 (표준)
          LONG : +1.5R 25% 청산 → +3.0R 33% 청산 (크게 천천히)

        +1R 도달 → 부분 청산 + 본전 가드 (or tier별 비율)
        +2R 도달 → 잔여 부분 청산 + 1R 잠금
        TP1 후 본전 회귀 → 잔여 청산 (BE 가드)
        TP2 후 1R 회귀 → 잔여 청산 (1R 잠금)

        Returns:
            None: 분할 청산만 수행 (포지션 유지)
            ("ES2_BE …", "TAKE_PROFIT") 또는
            ("ES2_LOCK_1R …", "TAKE_PROFIT"): 잔여 전체 청산 트리거
        """
        if pos.r_unit <= 0 or pos.quantity <= 1:
            return None
        if pos.side == "SHORT":
            return None  # SHORT(공매) 포지션은 Arbitrage에서 별도 처리

        # Phase F: tier 파라미터 조회 (기본 MID로 폴백)
        tier = pos.holding_tier or "MID"
        params = self._TIER_R_PARAMS.get(tier, self._TIER_R_PARAMS["MID"])

        entry_price = pos.entry_price
        tp1_target = entry_price + params["tp1_r"] * pos.r_unit
        tp2_target = entry_price + params["tp2_r"] * pos.r_unit
        ...
        if not pos.tp1_hit and current_price >= tp1_target:
            sell_qty = max(1, int(pos.quantity * params["tp1_pct"]))
            if sell_qty < pos.quantity:
                self._execute_partial_sell(pos, sell_qty, f"ES2A_TP1_{params['tp1_r']:.1f}R_{tier}")
                pos.tp1_hit = True
                pos.stop_loss = max(pos.stop_loss, round(entry_price))
                self._phase_stats.setdefault("es2a_tp1_partial", 0)
                self._phase_stats["es2a_tp1_partial"] += 1
                # Tier별 카운터
                self._phase_stats.setdefault(f"es2a_tier_{tier}", 0)
                self._phase_stats[f"es2a_tier_{tier}"] += 1
        # ES2B: TP2 @ +tp2_r R (TP1 이후만)
        elif pos.tp1_hit and not pos.tp2_hit and current_price >= tp2_target:
            sell_qty = max(1, int(pos.quantity * params["tp2_pct"]))
            if sell_qty < pos.quantity:
                self._execute_partial_sell(pos, sell_qty, f"ES2B_TP2_{params['tp2_r']:.1f}R_{tier}")
                pos.tp2_hit = True
                pos.stop_loss = max(pos.stop_loss, round(entry_price + 1.0 * pos.r_unit))
                self._phase_stats.setdefault("es2b_tp2_partial", 0)
                self._phase_stats["es2b_tp2_partial"] += 1
                self._phase_stats.setdefault(f"es2b_tier_{tier}", 0)
    ...
    def _check_exits_momentum(self):
        ...
        for code, pos in self.positions.items():
            ...
            # ── Phase E: R-Multiple 분할 청산 (헬퍼 호출) ──
            _rm_exit = self._check_r_multiple_partial_exit(pos, current_price)
            if _rm_exit is not None:
                exit_reason, exit_type = _rm_exit
