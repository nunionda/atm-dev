"""
Simulation Pydantic Models — 프론트엔드 TypeScript 인터페이스와 1:1 매칭.

Phase 4.C 분할: ats/simulation/engine.py → ats/simulation/models.py
"""
...
class SimPosition(BaseModel):
    ...
    # ── Phase E: R-Multiple 분할 청산 추적 ──
    r_unit: float = 0.0                    # 1R 가격 거리 (entry 시점 stop_distance 기록)
    tp1_hit: bool = False                  # +1R 부분 청산 (33%) 완료 여부
    tp2_hit: bool = False                  # +2R 부분 청산 (50% of remainder) 완료 여부
    # ── Phase F: Holding-Tier 분류 ──
    holding_tier: str = "MID"              # "LONG" | "MID" | "SHORT" — entry 시점 보유 기간 분류
