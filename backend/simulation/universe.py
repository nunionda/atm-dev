    ...
    def scan_with_recommendations(
                ...
                buy_list.append(
                    {
                        ...
                        "return_6m_raw": round(ret_6m, 4),  # ExitGuard 판정용 float
                        "action": "BUY",
                    }
                )
        ...
        for score, stock, df_slice in scored_stocks[: self.top_n]:
            if stock["code"] in active_codes:
                ...
                last_close = float(df_slice.iloc[-1]["close"])
                # 6개월 수익률 (HOLD도 ExitGuard 판정 위해 노출)
                idx_6m = max(0, len(df_slice) - 126)
                ret_6m_hold = (last_close - float(df_slice.iloc[idx_6m]["close"])) / float(
                    df_slice.iloc[idx_6m]["close"]
                )
                hold_list.append(
                    {
                        ...
                        "return_6m": f"{ret_6m_hold:+.1%}",
                        "return_6m_raw": round(ret_6m_hold, 4),
                        "action": "HOLD",
                    }
                )
