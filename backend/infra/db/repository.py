...
class Repository:
    ...
    def get_active_universe(self, market: Optional[str] = None) -> List[Universe]:
        """활성 유니버스 종목 목록을 반환한다.

        Args:
            market: None이면 전체 마켓 (기존 호환), 지정 시 해당 마켓만.
        """
        with self._session() as s:
            q = s.query(Universe).filter(Universe.is_active == 1)
            if market is not None:
                q = q.filter(Universe.market == market)
            return q.all()

    def get_universe_meta(self, stock_code: str, market: Optional[str] = None) -> Optional[dict]:
        """단일 종목의 rebalance 메타데이터를 dict로 반환.

        Args:
            stock_code: 종목 코드
            market: 마켓 명시 (멀티마켓 ticker 충돌 시 필수). None이면 임의의 한 row.
        """
        with self._session() as s:
            if market is not None:
                u = s.get(Universe, (stock_code, market))
            else:
                u = s.query(Universe).filter(Universe.stock_code == stock_code).first()
            if not u:
                return None
            return {
                ...
                "rebalance_action": u.rebalance_action,
                "rebalance_score": u.rebalance_score,
                "rebalance_rank": u.rebalance_rank,
                "return_6m": u.return_6m,
                "last_synced_at": u.last_synced_at,
            }

    def replace_active_universe(self, market: str, codes_meta: List[dict]) -> dict:
        """
        해당 market의 active universe를 원자적으로 교체한다 (멀티마켓 안전).

        1. market 내 모든 is_active=1 → 0
        2. codes_meta의 각 종목을 (stock_code, market) 복합키로 upsert + is_active=1

        같은 ticker가 다른 market(예: SP500 vs NDX)에 동시 존재해도 영향 없음.

        Args:
            market: 'sp500' | 'ndx' | 'kospi' 등
            codes_meta: 5절 참조

        Returns: {"deactivated": int, "activated": int}
        """
        now = datetime.now().isoformat()
        with self._session() as s:
            # 1단계: 기존 active 비활성화 (이 market만)
            deactivated = (
                s.query(Universe)
                .filter(Universe.market == market, Universe.is_active == 1)
                .update({"is_active": 0, "updated_at": now}, synchronize_session=False)
            )

            # 2단계: 새 active upsert (복합 PK 사용)
            activated = 0
            for meta in codes_meta:
                code = meta["stock_code"]
                existing = s.get(Universe, (code, market))
                fields = {
                    "stock_name": meta.get("stock_name") or (existing.stock_name if existing else code),
                    "sector": meta.get("sector") or (existing.sector if existing else None),
                    "is_active": 1,
                    "updated_at": now,
                    "rebalance_action": meta.get("rebalance_action"),
                    "rebalance_score": meta.get("rebalance_score"),
                    "rebalance_rank": meta.get("rebalance_rank"),
                ...
                if existing:
                    ...
                    s.add(Universe(stock_code=code, market=market, **fields))
                activated += 1

            s.commit()
            logger.info(
                "Universe replaced | market=%s | deactivated=%d | activated=%d",
                market, deactivated, activated,
            )
            return {"deactivated": deactivated, "activated": activated}

    def upsert_rebalance_sync_log(self, **kwargs) -> int:
        """RebalanceSyncLog 행을 추가한다. sync_id 반환."""
        with self._session() as s:
            row = RebalanceSyncLog(
                market=kwargs["market"],
                scan_date=kwargs["scan_date"],
                trigger=kwargs.get("trigger", "AUTO"),
                top_n=kwargs.get("top_n", 0),
                buy_count=kwargs.get("buy_count", 0),
                hold_count=kwargs.get("hold_count", 0),
                kept_count=kwargs.get("kept_count", 0),
                total_active=kwargs.get("total_active", 0),
                protected_count=kwargs.get("protected_count", 0),
                target_codes_json=kwargs.get("target_codes_json"),
    ...
    def get_latest_rebalance_sync(self, market: str) -> Optional[dict]:
        """해당 market의 최근 sync 로그를 반환."""
        with self._session() as s:
            row = (
                s.query(RebalanceSyncLog)
                .filter(RebalanceSyncLog.market == market)
                .order_by(RebalanceSyncLog.created_at.desc())
                .first()
            )
            if not row:
                return None
    ...
    def list_rebalance_sync_logs(self, market: Optional[str] = None, limit: int = 7) -> List[dict]:
        """sync 로그 최근 N건을 반환 (drawer용)."""
        with self._session() as s:
            q = s.query(RebalanceSyncLog).order_by(RebalanceSyncLog.created_at.desc())
            if market:
                q = q.filter(RebalanceSyncLog.market == market)
    ...
    def upsert_universe(self, stock_code: str, stock_name: str,
        ...
        """유니버스 종목을 추가하거나 업데이트한다 (멀티마켓: 복합 PK)."""
        with self._session() as s:
            existing = s.get(Universe, (stock_code, market))
            now = datetime.now().isoformat()
            if existing:
