...
class SP500FuturesStrategy(BaseStrategy):
    ...
    def _evaluate_entry(
        ...
        # P3-2: 종목별 entry threshold boost (Gold 같은 특수 자산 더 엄격하게)
        ...
        sl, tp = self._calculate_sl_tp(current_price, atr, is_long, float(curr.get("adx", 0)), ticker=code)
    ...
    def generate_futures_signal(
        ...
        sl, tp = self._calculate_sl_tp(current_price, atr, is_long, adx, ticker=code)
    ...
    def _check_ev_gate(self) -> bool:
        ...
        P1-3: warmup 정책 추가.
          - 표본 < ev_min_trades(30)일 때:
            ev_warmup_block_entries=False (default) → 통과 (backward compat, backtester가 표본 채울 수 있음)
            ev_warmup_block_entries=True → 차단 (라이브 안전 — paper trading으로 warmup 후 활성화)
        """
        ev = self._calculate_ev()
        if ev is None:
            # 표본 부족 (< ev_min_trades)
            if getattr(self.fc, "ev_warmup_block_entries", False):
                logger.info(
                    "EV WARMUP BLOCK | trades=%d/%d (block_entries=True)",
                    len(self._trade_history), self.fc.ev_min_trades,
                )
                return False
            return True
        if ev <= 0:
    ...
    def _get_ticker_threshold_boost(self, ticker: Optional[str]) -> float:
        """
        P3-2: 종목별 entry_threshold boost.

        Gold 같은 안전자산은 ES-style 모멘텀 알고리즘에 fit 안 됨 → 더 엄격한 threshold로 false positive 줄임.
        디폴트 0 (boost 없음). config의 entry_threshold_boost_map에서 ticker 매칭 시 가산.
        """
        if not ticker:
            return 0.0
    ...
    def _get_atr_mult(self, ticker: Optional[str], adx: float) -> float:
        """
        P0-1: 종목별 ATR multiplier 선택.

        atr_mult_map에 ticker 매칭이 있으면 (strong, weak) 튜플에서 ADX 기반 선택.
        없으면 글로벌 sl_atr_mult_strong / sl_atr_mult로 폴백 (기존 ES=F 동작 보존).
        """
        is_strong = adx >= self.fc.adx_threshold
        mp = getattr(self.fc, "atr_mult_map", None) or {}
        if ticker and ticker in mp:
            strong_mult, weak_mult = mp[ticker]
            return strong_mult if is_strong else weak_mult
        # Fallback: 글로벌 디폴트
        return self.fc.sl_atr_mult_strong if is_strong else self.fc.sl_atr_mult
    ...
    def _calculate_sl_tp(
        ...
        P0-1: ticker별 ATR multiplier 적용 (NQ 좁게, GC 넓게).
        Dynamic ATR 배수: ADX < threshold → weak, ≥ threshold → strong.
        하드 손절: 절대 한도 (sl_hard_pct, 디폴트 ±3%).
        """
        sl_mult = self._get_atr_mult(ticker, adx)
    ...
    def _calculate_position_size(
        ...
        # P1-4: BR-P02 max_weight cap (notional ≤ max_weight_pct × equity)
        # 단일 계약 notional이 cap을 초과해도 최소 1 계약은 진입 보장 (사용자 책임).
        max_weight = getattr(self.fc, "max_weight_pct", 0.15)
        if max_weight > 0 and equity > 0:
            max_notional = equity * max_weight
            notional = contracts * entry_price * multiplier
            if notional > max_notional:
                capped = max(1, int(max_notional / (entry_price * multiplier)))
                if capped < contracts:
                    logger.info(
                        "BR-P02 CAP | contracts %d → %d | notional $%.0f > cap $%.0f (equity × %.0f%%)",
                        contracts, capped, notional, max_notional, max_weight * 100,
                    )
                    contracts = capped
    ...
    def _check_atr_stop_loss(
        ...
        sl, _ = self._calculate_sl_tp(entry_price, atr, is_long, adx, ticker=pos.stock_code)
    ...
    def _check_atr_take_profit(
        ...
        _, tp = self._calculate_sl_tp(entry_price, atr, is_long, 0, ticker=pos.stock_code)
