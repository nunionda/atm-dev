...
class ESFIntradayStrategy:
    ...
    def compute_magnetic_ma(self, df: pd.DataFrame) -> dict:
        ...
        for period in [9, 20, 50, 100, 200]:
            ...
            for ma_type in ["SMA", "EMA"]:
                ...
                # P3-4: 마지막 lookback 봉을 완전 제외하지 않고 가능한 데이터까지 부분 활용.
                # 기존: `range(period, len(indices) - lookback)` → 최근 10봉 deviation이 통계에서 누락
                #      → 최근 시장 변화 미반영 (E4 진단)
                # 개선: 마지막 1봉(lookback 불가)만 제외하고, available_lookback = min(10, 잔여 봉수)
                indices = df.index.tolist()
                for idx_pos in range(period, len(indices) - 1):
                    if not deviations.iloc[idx_pos]:
                        continue
                    available_lookback = min(lookback, len(indices) - 1 - idx_pos)
                    if available_lookback <= 0:
                        continue  # lookback 불가
                    total_deviations += 1
                    for j in range(1, available_lookback + 1):
    ...
    def _determine_direction(
        ...
        P3-1: 임계값 3 → 2로 완화 (NEUTRAL regime에서 IMBALANCE 마켓상태가 적어
        score 3 도달 어려움 → 60일 trade 3건 한계의 본질적 원인).
        후속 필터 (4-Layer scoring, Grade threshold, EMA veto, time filter, momentum gate)가
        false positive entry를 제거하므로 임계 완화 안전.

        합계 ≥ 2 → LONG, ≤ -2 → SHORT, 그 외 NEUTRAL.
    ...
    def _score_amt_location(
        ...
        if is_long and market_state == "IMBALANCE_BULL":
            ...
            # P4-1: BALANCE 시 boost — 1/2 → 0.7 (NEUTRAL regime이 대부분인 시장에서 trade 빈도 회복)
            score += round(amt_max * 0.7, 1)
