    ...
    def scan_exit_signals(
        ...
        for pos in positions:
            ...
            if is_short:
                ...
                # Phase B: LONG 포지션만 ExitGuard 보호 (SHORT는 rebalance HOLD bias와 반대)
                _mr_protected = bool(
                    self.exit_guard
                    and self.exit_guard.is_es2_protected(pos, self.market_id)[0]
                )
                if ma20 and current_price > ma20:
                    if _mr_protected:
                        logger.info("ES_MR_TP_SKIPPED_PROTECTED LONG | %s | Price > MA20 | pnl=%.2f%%",
                ...
                if rsi_val is not None and rsi_val > 60:
                    if _mr_protected:
                        logger.info("ES_MR_TP_SKIPPED_PROTECTED LONG | %s | RSI > 60 | pnl=%.2f%%",
                                    pos.stock_name, pnl_pct * 100)
                    else:
                        exit_signals.append(ExitSignal(
                            stock_code=pos.stock_code,
