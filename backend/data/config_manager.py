...
class OrderConfig:
    ...
    # Option A — 라이브 매매 PnL 정확성 (CLAUDE.md 거래비용)
    # commission_pct: 양쪽 합산 round-trip = 2 × 0.00015 = 0.03%
    # slippage_pct: 실전 KIS는 broker fill에 자연 반영 → 0으로 두기 권장.
    #               모의/mock은 0.001 (0.1%/side)로 설정 필요.
    commission_pct: float = 0.00015      # 0.015% / side (한 방향)
    slippage_pct: float = 0.0            # 실전 default 0 (broker fill 반영); 모의 시 0.001
...
class UniverseConfig:
    ...
    # Phase A — Rebalance sync 정책
    auto_sync_enabled: bool = True           # 매일 장 마감 후 자동 sync
    top_n: int = 10                          # BUY top_n (BR-P01 BULL max position 10과 일치)
    min_universe_size: int = 3               # sync 결과 합집합이 이보다 적으면 거부
    always_active: List[str] = field(default_factory=list)  # 화이트리스트 (예: SPY/QQQ)

    # Phase B — ES2 Winner 보호
    protect_winners_from_es2: bool = False   # 기본 OFF — Phase B 활성화 시 true
    protect_min_return_6m: float = 0.0       # 0.0 = 0%부터 보호 (사용자 정책 "6개월 이상 수익")
    protect_min_score: float = 0.0           # 0~100 모멘텀 점수 최소
...
class SP500FuturesConfig:
    ...
    # 손절 설정 (전역 디폴트 — atr_mult_map에 ticker 없을 때 fallback)
    sl_atr_mult: float = 2.0
    sl_atr_mult_strong: float = 1.5
    sl_hard_pct: float = 0.03

    # P3-2: 종목별 entry_threshold boost (Gold 같은 특수 자산은 더 엄격하게)
    # ex: 알고리즘이 ES=F-style 모멘텀에 fit → GC=F는 false positive 많음 → +10 boost
    # 값은 baseline threshold(regime별 50/55/65)에 가산됨. 0이면 default.
    entry_threshold_boost_map: Dict[str, float] = field(default_factory=lambda: {
        "GC=F": 10.0,
        "MGC=F": 10.0,
    })

    # P0-1: 종목별 ATR multiplier 분리 (NQ MDD -52% → over-fit fix)
    # 키: ticker, 값: (strong_mult, weak_mult) — strong은 ADX>=threshold, weak는 else.
    # 백테스트로 튜닝된 값 (2024-06 ~ 2026-05):
    #   NQ: 좁게 (Return -10.7% → +16.2%, MDD -52% → -45%)
    #   CL: 약간 좁게 (이미 5거래만)
    #   GC: 디폴트가 최적 (넓히면 역효과 검증됨)
    atr_mult_map: Dict[str, Tuple[float, float]] = field(default_factory=lambda: {
        "ES=F":  (1.5, 2.0),
        "MES=F": (1.5, 2.0),
        "NQ=F":  (1.0, 1.3),
        "MNQ=F": (1.0, 1.3),
    ...
    # P1-4: BR-P02 종목당 최대 비중 (notional ≤ max_weight_pct × equity)
    # CLAUDE.md BR-P02 = 15%. notional 초과 시 contracts cap (최소 1 계약 보장).
    # 작은 자본 + full futures는 1 계약 notional이 cap 초과 가능 → micro 전환 권장.
    ...
    ev_lookback: int = 30
    # P1-3: EV Engine warmup
    # - ev_min_trades 5 → 30 (충분한 통계적 표본 필요. 5는 false positive 위험 큼)
    # - ev_warmup_block_entries: warmup 동안 진입 차단 여부 (default False = backward compat).
    #   True이면 백테스터는 warmup 채우지 못해 trade 0가 됨 → 라이브 환경에서만 활성화 권장
    #   (paper trading으로 warmup 채운 뒤 라이브 전환).
    ev_min_trades: int = 30
    ev_warmup_block_entries: bool = False
...
class ESFIntradayConfig:
    ...
    grade_c_threshold: float = 20.0  # P4-1: 25 → 20 (추가 완화로 boundary trade 확대, contract_mult 0.25로 보수적)
    ...
    # P1-2: Regime-aware Grade A threshold (이전 모두 50으로 동일했음 → 모든 regime에서 진입 제한)
    # 실측 score 분포 분석:
    #   60일 NEUTRAL regime trades 평균 score 40-48 (Grade A 50 미달 → 전부 Grade B로 entry)
    #   → NEUTRAL/BEAR threshold를 실측 score 범위에 맞춰 낮춤 (Grade A 진입 확보, mult 1.0 활용)
    # 의도: BULL 보수적 / NEUTRAL·BEAR는 실측 데이터 기반 적정 threshold로 진입 빈도 회복
    # Layer score 자체 개선은 P3 (E3 AMT, E4 Magnetic MA) 영역
    regime_bull_threshold: float = 55.0     # Grade A in BULL (보수적; +5 bonus로 도달 가능)
    # P4-1: NEUTRAL 45 → 40 추가 완화 + L1 BALANCE boost로 NEUTRAL regime trade 빈도 회복
    regime_neutral_threshold: float = 40.0  # Grade A in NEUTRAL
    regime_bear_threshold: float = 40.0     # Grade A in BEAR (SHORT counter-trend 확장)
...
class ConfigManager:
    ...
    def load(self) -> ATSConfig:
        ...
        config.universe = UniverseConfig(
            ...
            # Phase A/B — Rebalance sync 정책 (없으면 dataclass 기본값)
            auto_sync_enabled=uni.get("auto_sync_enabled", True),
            top_n=uni.get("top_n", 10),
            min_universe_size=uni.get("min_universe_size", 3),
            always_active=uni.get("always_active", []),
            protect_winners_from_es2=uni.get("protect_winners_from_es2", False),
            protect_min_return_6m=uni.get("protect_min_return_6m", 0.0),
            protect_min_score=uni.get("protect_min_score", 0.0),
        )
        ...
        if sp500f:
            # tuple/dict 필드는 별도 처리
            excluded = ("rsi_long_range", "rsi_short_range", "atr_mult_map")
            scalar = {
                k: sp500f[k] for k in SP500FuturesConfig.__dataclass_fields__
                if k in sp500f and k not in excluded
            }
            ...
            # P0-1: 종목별 ATR map (YAML에서 override 가능)
            if "atr_mult_map" in sp500f:
                # YAML은 list 표현이므로 tuple 변환
                config.sp500_futures.atr_mult_map = {
                    k: tuple(v) for k, v in sp500f["atr_mult_map"].items()
                }
            # P3-2: 종목별 entry threshold boost (YAML override)
            if "entry_threshold_boost_map" in sp500f:
