...
class LiveDataService:
    ...
    def __init__(self, event_bus: SSEEventBus, paper_engine=None):
        self._event_bus = event_bus
        self._paper_engine = paper_engine  # FuturesPaperEngine (optional)
    ...
    async def _price_loop(self):
        ...
        while self._is_running:
            try:
                ...
                # OPEN paper position이 있는 ticker 자동 구독
                if self._paper_engine is not None:
                    try:
                        for t in self._paper_engine.get_open_tickers():
                            self._subscribed_tickers.add(t)
                    except Exception as e:
                        logger.warning("paper open_tickers fetch failed: %s", e)
        ...
        try:
            ...
            for ticker in tickers:
                try:
                    ...
                    # Paper engine SL/TP tick (sync 호출 → 청산 시 sync publish)
                    if self._paper_engine is not None:
                        try:
                            await asyncio.to_thread(self._paper_engine.tick, ticker, current)
                        except Exception as e:
                            logger.warning("paper tick failed for %s: %s", ticker, e)
