...
class WatchlistItem(BaseModel):
    """단일 워치리스트 종목."""
    code: str         # "NVDA" 또는 "005930"
    ticker: str       # "NVDA" 또는 "005930.KS"
    name: str         # 표시 이름


class SetWatchlistRequest(BaseModel):
    """동적 워치리스트 주입 요청."""
    market: str                          # kospi | sp500 | ndx
    watchlist: List[WatchlistItem]       # 신규 워치리스트 (예: 추천 10종목)
    strategy_mode: Optional[str] = None  # 같이 변경 (선택)
...
@sim_router.post("/sim/set-watchlist")
async def set_watchlist(req: SetWatchlistRequest):
    """특정 마켓의 워치리스트를 동적으로 교체하고 시뮬을 재시작한다.

    1. 기존 마켓 시뮬 정지 (포지션 강제 청산)
    2. MARKET_CONFIG의 watchlist를 req.watchlist로 in-memory 교체
    3. 새 워치리스트로 데이터 재로드 + 시뮬 재시작

    예: 추천 10종목으로 시뮬 가동 ("Stock Strategy 통합매매엔진").
    """
    ctrl = _get_controller()

    if len(req.watchlist) < 1:
        return {"status": "error", "detail": "watchlist must have at least 1 item"}

    # MARKET_CONFIG의 워치리스트를 in-memory로 교체
    from simulation.watchlists import MARKET_CONFIG
    if req.market not in MARKET_CONFIG:
        return {"status": "error", "detail": f"Unknown market: {req.market}"}

    new_watchlist = [
        {"code": w.code, "ticker": w.ticker, "name": w.name}
        for w in req.watchlist
    ]
    MARKET_CONFIG[req.market]["watchlist"] = new_watchlist

    # 기존 엔진 정지 + 강제 청산
    engine = ctrl.get_engine(req.market)
    if engine and engine._is_running:
        try:
            engine.force_liquidate_all_immediate()
        except Exception:
            pass
        await ctrl.stop_market(req.market)

    # 엔진 인스턴스 제거 (start_market이 새로 생성하도록)
    ctrl._engines.pop(req.market, None)

    # 새 워치리스트로 시뮬 시작
    result = await ctrl.start_market(
        req.market,
        strategy_mode=req.strategy_mode or "multi",
    )
    return {
        **result,
        "watchlist_size": len(new_watchlist),
        "watchlist": [w["name"] for w in new_watchlist],
    }
