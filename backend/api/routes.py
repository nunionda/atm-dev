...
# ── Deprecated / delisted ticker aliases ───────────────────────────────
# yfinance에서 사라진 심볼을 후속 심볼로 자동 매핑 (사용자 입력/캐시 호환).
DEPRECATED_TICKER_ALIASES: dict[str, str] = {
    "DX=F": "DX-Y.NYB",  # USD Index futures → ICE Dollar Index
}
...
async def analyze_ticker(ticker: str, period: str = "1mo", interval: str = "1d"):
    ...
    """
    try:
        # ── Deprecated ticker alias (e.g., DX=F → DX-Y.NYB) ──
        if ticker in DEPRECATED_TICKER_ALIASES:
            ticker = DEPRECATED_TICKER_ALIASES[ticker]
...
async def market_overview_endpoint():
    ...
    5분 캐싱 적용. 4초 timeout + cache fallback + 빈 stub으로 frontend hang 방지.
    """
    from .market_overview import _cache, MARKET_INDICES
    try:
        # 4초 안에 응답 없으면 fallback
        ...
        # Stale cache 있으면 사용
        if _cache.get("data"):
            return {**_cache["data"], "cache_stale": True}
        # 캐시도 없으면 stub (빈 indices) — 프론트는 SSE 통해 채워질 때까지 대기
        return {
            "indices": [
                {
                    "symbol": idx["symbol"],
            ...
            "regime": {"regime": "NEUTRAL", "label": "Pending", "label_kr": "데이터 로딩", "score": 0, "signals": []},
            "updated_at": "",
            "pending": True,
        }
