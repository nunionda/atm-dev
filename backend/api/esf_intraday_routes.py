...
# Walk-Forward
_wf_in_progress = False
_wf_cache: Optional[dict] = None
_wf_progress: float = 0.0
...
async def run_walk_forward(req: WalkForwardRequest):
    """자산군별 contract life cycle에 맞춘 walk-forward 백테스트.

    Equity Index(ES/MES/NQ/MNQ) → 3개월 윈도우 × 4
    Energy(CL) / Metal(GC) → 1개월 윈도우 × 12
    """
    global _wf_in_progress, _wf_cache, _wf_progress

    if req.ticker not in WALK_FORWARD_TICKERS:
        raise HTTPException(
            status_code=400,
            detail=f"Unsupported ticker for walk-forward: {req.ticker}. "
                   f"Valid: {sorted(WALK_FORWARD_TICKERS)}",
        )
    if req.initial_equity <= 0:
        raise HTTPException(status_code=400, detail="initial_equity must be positive")
    if _wf_in_progress:
        raise HTTPException(status_code=409, detail="Walk-Forward already in progress")

    _wf_in_progress = True
    _wf_progress = 0.0

    def _run_sync():
        global _wf_cache, _wf_in_progress, _wf_progress
        try:
            import copy
            from backtest.walk_forward_engine import WalkForwardEngine, to_dict

            cfg = copy.deepcopy(_get_config())

            def _on_progress(pct: float):
                global _wf_progress
                _wf_progress = pct * 100.0
            ...
            logger.error("Walk-Forward error: %s", str(e), exc_info=True)
            _wf_cache = {"error": str(e)}
            raise
        finally:
            _wf_in_progress = False
...
@esf_router.get("/esf/backtest/walk-forward/status")
async def walk_forward_status():
    """Walk-Forward 진행 상태."""
    status = "idle"
    if _wf_in_progress:
        status = "running"
    elif _wf_cache is not None:
        status = "completed"
    return {"status": status, "progress": round(_wf_progress, 1)}


@esf_router.get("/esf/backtest/walk-forward/result")
async def walk_forward_result():
    """캐시된 Walk-Forward 결과 조회."""
    if _wf_cache is None:
        raise HTTPException(status_code=404, detail="No walk-forward result available")
    return _sanitize_for_json(_wf_cache)
