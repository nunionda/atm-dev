...
from .esf_journal_routes import esf_journal_router, _get_recorder
from .esf_paper_routes import esf_paper_router, set_engine as set_paper_engine
from .rebalance_sync_routes import rebalance_sync_router
...
# Paper trading engine (live_data와 EventBus를 공유)
_paper_db = Database()
_paper_db.init_tables()


def _paper_publish_sync(event_type: str, data):
    """동기 publish 어댑터 — futures_paper_engine은 sync, EventBus는 async."""
    try:
...
paper_engine = FuturesPaperEngine(
    database=_paper_db,
    recorder=_get_recorder(),
    publish=_paper_publish_sync,
    max_concurrent=3,
)
set_paper_engine(paper_engine)

live_data = LiveDataService(event_bus, paper_engine=paper_engine)
...
app.include_router(esf_paper_router, prefix="/api/v1")
app.include_router(rebalance_sync_router, prefix="/api/v1")
