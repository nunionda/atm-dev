...
class PositionManager:
    ...
    def close_position(
        ...
        Option A (라이브 PnL 정확성):
          gross_pnl = (exit - entry) × qty
          cost = (entry_amount + exit_amount) × (commission_pct + slippage_pct)
          pnl = gross_pnl - cost
        CLAUDE.md 거래비용: commission 0.015%/side, slippage 0.1%/side (실전 broker fill에 자연 반영 시 0).
        """
        ...
        # Option A: 명시적 round-trip 비용 차감
        oc = getattr(self.config, "order", None)
        commission_pct = getattr(oc, "commission_pct", 0.00015) if oc else 0.00015
        slippage_pct = getattr(oc, "slippage_pct", 0.0) if oc else 0.0
        cost_pct = commission_pct + slippage_pct

        entry_amount = entry_price * quantity
        exit_amount = exit_price * quantity
        gross_pnl = (exit_price - entry_price) * quantity
        cost = (entry_amount + exit_amount) * cost_pct
        pnl = gross_pnl - cost
        ...
        logger.info(
            "Position CLOSED | id=%s | %s | gross=%.0f cost=%.0f pnl=%.0f원 (%.2f%%) | reason=%s",
            position_id, pos.stock_name, gross_pnl, cost, pnl, pnl_pct * 100, exit_reason,
        )
