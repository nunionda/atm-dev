...
# Phase A/B — Rebalance sync + ExitGuard (optional)
try:
    from core.rebalance_sync import RebalanceSyncJob
    from risk.exit_guard import ExitGuard
except Exception:  # 백테스트/스크립트 컨텍스트에서 import 실패해도 무방
    RebalanceSyncJob = None  # type: ignore
    ExitGuard = None  # type: ignore
...
class MainLoop:
    ...
    def __init__(
        ...
        market_id: str = "kospi",
        rebalance_sync=None,        # Optional[RebalanceSyncJob]
        exit_guard=None,            # Optional[ExitGuard]
    ):
        self.config = config
        ...
        # Phase A/B: market_id + 선택적 Rebalance sync + ExitGuard
        self.market_id = market_id
        self.rebalance_sync = rebalance_sync
        self.exit_guard = exit_guard

        # 전략에 ExitGuard 주입 (라이브 only; 백테스트는 None 유지)
        if self.exit_guard is not None and hasattr(self.strategy, "set_exit_guard"):
            self.strategy.set_exit_guard(self.exit_guard, market_id=self.market_id)
    ...
    def on_market_close(self):
        """장 마감 후 처리 (보유일수 증가, 리포트 생성, Rebalance sync)."""
        ...
        # Phase A: Rebalance → Universe 동기화 (auto_sync_enabled일 때)
        if self.rebalance_sync is not None and getattr(self.config.universe, "auto_sync_enabled", False):
            try:
                result = self.rebalance_sync.run(market=self.market_id, trigger="AUTO", commit=True)
                if self.exit_guard is not None:
                    self.exit_guard.invalidate_cache(market=self.market_id)
                # 알림
                if result.rejected:
                    self.notifier.send_message(
                        f"⚠️ Rebalance sync rejected | {self.market_id} | "
                        f"{result.reject_reason}", level="WARN",
                    )
                elif result.error:
                    self.notifier.send_message(
                        f"❌ Rebalance sync failed | {self.market_id} | {result.error}",
                        level="ERROR",
                    )
                else:
                    self.notifier.send_message(
                        f"🔄 Universe synced ({self.market_id}) | "
                        f"BUY={result.buy_count} HOLD={result.hold_count} KEEP={result.kept_count} | "
                        f"protected={result.protected_count}",
                    )
            except Exception as e:
                logger.error("Rebalance sync hook failed: %s", e, exc_info=True)
