...
class IntradayBacktester:
    ...
    def _check_exit(
        ...
        # P1-1: Entry sl_level과 Exit hard-stop 통일.
        # Entry는 sl = max(atr_sl, entry*(1-sl_hard_pct))로 hard_cap이 이미 적용됨.
        # → 정상 시 ES_ATR_SL이 먼저 발동되어야 함 (entry_sl ≥ hard_cap_price).
        # ES_HARD는 안전판: ① 갭 다운으로 봉 low가 hard_cap_price 이하로 내려간 경우
        # ② atr 측정 실패로 entry_sl이 비정상적으로 멀게 잡힌 경우.
        # close 기준이 아닌 low/high(intraday OHLC)로 평가 — 갭/스파이크 정확 캐치.
        if position.direction == "LONG":
            hard_cap_price = position.entry_price * (1 - self.ic.sl_hard_pct)
            if low <= hard_cap_price:
                return ExitReason.HARD_STOP.value
        else:
            hard_cap_price = position.entry_price * (1 + self.ic.sl_hard_pct)
            if high >= hard_cap_price:
                return ExitReason.HARD_STOP.value

        # ES_ATR_SL: ATR 기반 손절 (entry 시 결정된 stop_loss 레벨)
