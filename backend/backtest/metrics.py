...
class PhaseStats:
    ...
    # ── Phase E: R-Multiple 분할 청산 ──
    es2a_tp1_partial: int = 0   # +1R 도달 → 33% 부분 청산 + 본전 가드
    es2b_tp2_partial: int = 0   # +2R 도달 → 50% 부분 청산 + 1R 잠금
    es_disp_partial_sell: int = 0  # BULL 이격도 부분 청산 (기존 카운터 명시화)
    # ── Phase F: Holding-Tier 분류 카운터 ──
    tier_LONG_entries: int = 0    # 강한 추세 — TP1 1.5R/25%, TP2 3.0R/33%
    tier_MID_entries: int = 0     # 표준 스윙 — TP1 1.0R/33%, TP2 2.0R/50%
    tier_SHORT_entries: int = 0   # 약한 시그널 — TP1 0.8R/50%, TP2 1.5R/50%
    es2a_tier_LONG: int = 0
    es2a_tier_MID: int = 0
    es2a_tier_SHORT: int = 0
