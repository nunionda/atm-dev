...
export function BacktestSection({ activeMarket }: BacktestSectionProps) {
    ...
    const [strategy, setStrategy] = useState<'multi' | 'momentum' | 'smc' | 'breakout_retest' | 'mean_reversion' | 'arbitrage'>('multi');
    ...
    const isMulti = result?.strategy === 'multi';
            ...
            >
                <div className="backtest-section-title">
                    ...
                    {m && (
                        <>
                            {isMulti && (
                                <span className="backtest-pill strategy-smc">통합매매엔진 (Multi)</span>
                            )}
                            {isSmc && (
                ...
                <div className="backtest-content glass-panel">
                    ...
                    <div className="backtest-controls">
                        <div className="backtest-strategy-selector">
                            <button
                                className={`strategy-btn ${strategy === 'multi' ? 'active' : ''}`}
                                onClick={() => setStrategy('multi')}
                                disabled={isRunning}
                                title="레짐별 7개 전략 동적 배분 — Operations 시뮬과 동일 (권장)"
                            >
                                Stock Strategy (Multi)
                            </button>
                            <button
                                                    ...
                                                    <tr key={`${t.stock_code}-${t.entry_date}-${i}`}>
                                                        <td className="code-cell">
                                                            <div className="stock-cell">
                                                                <span className="stock-name">{t.stock_name || '—'}</span>
                                                                <span className="stock-code-sub">{t.stock_code}</span>
                                                            </div>
                                                        </td>
                                                        <td>{t.entry_date}</td>
                                                        <td>{t.exit_date}</td>
