    ...
    return (
        <div className="chart-toolbar">
            ...
            <div className="toolbar-section">
                ...
                {Object.entries(MA_CONFIG).map(([rawKey, conf]) => {
                    const key = rawKey as keyof OverlayState;
                    const isActive = activeOverlays[key];
                    return (
                        <button
                            key={rawKey}
                            className={`tf-btn ${isActive ? 'active' : ''}`}
                            onClick={() => onOverlayToggle(key)}
                            style={isActive ? { color: conf.color, boxShadow: `0 0 0 1px ${conf.color}40` } : {}}
