...
export function TechnicalChart({
    ...
    useEffect(() => {
        ...
        if (chartType === 'line') {
            ...
            // chartData는 line 161에서 이미 dedupByTime 적용 — map은 time을 유지하므로 재dedup 불필요
        ...
        // ── Subchart helpers (RSI / MACD / ADX) ──
        // Phase D2-Ext: 공통 createChart + height=160 패턴을 묶어 76줄 → 효과적 분기.
        const createSubchart = (container: HTMLDivElement): IChartApi => {
        ...
        // resize handler에서 참조하므로 if 블록 밖에 hoist
        ...
        if (activeSubcharts.rsi && rsiContainerRef.current) {
            rsiChart = createSubchart(rsiContainerRef.current);
        ...
        if (activeSubcharts.macd && macdContainerRef.current) {
            macdChart = createSubchart(macdContainerRef.current);
        ...
        if (activeSubcharts.adx && adxContainerRef.current) {
            adxChart = createSubchart(adxContainerRef.current);
        ...
        // Phase D2-Ext-2: cascade-guarded sync 로직을 inner helper로 묶음.
        // 메인 → sub, sub → 메인 + 다른 sub 양방향 동기화. `syncing` flag로 재귀 방지.
        const bindTimeSync = (main: IChartApi, subs: IChartApi[]) => {
            let syncing = false;
            main.timeScale().subscribeVisibleTimeRangeChange((range) => {
                if (syncing || !range) return;
                syncing = true;
                subs.forEach(c => c.timeScale().setVisibleRange(range));
                syncing = false;
            });
            subs.forEach(srcChart => {
                srcChart.timeScale().subscribeVisibleTimeRangeChange((range) => {
                    if (syncing || !range) return;
                    syncing = true;
                    main.timeScale().setVisibleRange(range);
                    subs.forEach(t => { if (t !== srcChart) t.timeScale().setVisibleRange(range); });
                    syncing = false;
                });
            });
        };
        bindTimeSync(chart, subcharts);

        // === Crosshair Sync + OHLCV Overlay ===
        // Phase D2-Final: inner helper로 캡슐화. main + subs 간 양방향 sync,
        // OHLCV overlay dispatch, 측정/그리기 도구 라이브 업데이트를 한 closure에 묶음.
        const bindCrosshair = () => {
        ...
        // Phase D2-Final: 클릭 시 두 가지 동작 분기 — 그리기 도구가 활성이면 anchor/완성 처리,
        // 측정 모드면 anchored/locked/idle 사이클. inner helper로 캡슐화.
