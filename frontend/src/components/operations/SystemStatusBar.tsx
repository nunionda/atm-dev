...
import { Activity, Clock, Wallet, BarChart3, TrendingUp } from 'lucide-react';
        ...
        <div className="system-status-bar glass-panel">
            ...
            <div className="status-right">
                <div className="status-metric">
                    ...
                    <span className="metric-value">{sym}{Math.round(displayState.cash).toLocaleString()}</span>
                </div>
                <span className="status-divider" />
                <div className="status-metric">
                    <TrendingUp size={14} />
                    <span className="metric-label">투자금액</span>
                    <span className="metric-value">
                        {sym}{Math.round(displayState.invested ?? (displayState.total_equity - displayState.cash)).toLocaleString()}
                        <span className="metric-sub">
                            {' '}({displayState.total_equity > 0
                                ? (((displayState.invested ?? (displayState.total_equity - displayState.cash)) / displayState.total_equity) * 100).toFixed(0)
                                : 0}%)
                        </span>
                    </span>
                </div>
                ...
                <div className="status-metric">
                    ...
                    <span className="metric-value">{sym}{Math.round(displayState.total_equity).toLocaleString()}</span>
                </div>
                <span className="status-divider" />
                <div className="status-metric">
                    <Activity size={14} />
