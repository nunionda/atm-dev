        ...
        <div className="position-table-wrapper glass-panel">
            <div className="section-header">
                ...
                <div className={`total-pnl ${totalPnl >= 0 ? 'positive' : 'negative'}`}>
                    합계 {totalPnl >= 0 ? '+' : ''}{sym}{totalPnl.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                </div>
            </div>
                    ...
                    <tbody>
                        {displayPositions.map(pos => {
                            ...
                            return (
                                <tr
                                    key={pos.id}
                                    className={`position-row-clickable${isHighlighted ? ' row-highlight' : ''}`}
                                    onClick={() => navigate(`/dashboard?ticker=${encodeURIComponent(pos.stock_code)}&market=${market}`)}
                                    title="클릭 시 Dashboard 차트로 이동"
                                ...
                                >
                                    ...
                                    <td className="num">{pos.quantity.toLocaleString(undefined, { maximumFractionDigits: 0 })}</td>
                                    <td className="num">{sym}{pos.entry_price.toLocaleString(undefined, { maximumFractionDigits: 0 })}</td>
                                    <td className="num">{sym}{pos.current_price.toLocaleString(undefined, { maximumFractionDigits: 0 })}</td>
                                    ...
                                    <td className={`num ${isProfit ? 'text-profit' : 'text-loss'}`}>
                                        {isProfit ? '+' : ''}{sym}{pos.pnl.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                                    </td>
                                    <td className="num stop-price">{sym}{pos.stop_loss.toLocaleString(undefined, { maximumFractionDigits: 0 })}</td>
                                    <td className="num tp-price">{sym}{pos.take_profit.toLocaleString(undefined, { maximumFractionDigits: 0 })}</td>
