...
import {
  ...
  openPaperPosition as apiOpenPaperPosition,
  getOpenPaperPositions,
  closePaperPosition as apiClosePaperPosition,
  getPaperRiskStatus,
  triggerWalkForward,
  type ESFAnalysis,
  type ESFCandle,
...
export interface ESFuturesState {
  ...
  placePaperTrade: (contracts?: number) => Promise<void>;
  closePaperTrade: (positionId: string, price?: number) => Promise<void>;
  refreshPaperPositions: () => void;

  // Walk-Forward Backtest
  wfTicker: string;
  setWfTicker: (t: string) => void;
  wfIsMicro: boolean;
  setWfIsMicro: (v: boolean) => void;
  wfRunning: boolean;
  wfProgress: number;
...
export function useESFuturesScalp(): ESFuturesState {
  ...
  const [openPaperPositions, setOpenPaperPositions] = useState<FuturesPaperPosition[]>([]);
  const [paperRisk, setPaperRisk] = useState<PaperRiskStatus | null>(null);
  const [placingPaperOrder, setPlacingPaperOrder] = useState(false);
  const [paperOrderError, setPaperOrderError] = useState<string | null>(null);
  ...
  const activePosition = useMemo(() => {
    return openPaperPositions.find(p => p.ticker === effectiveTicker) ?? null;
  }, [openPaperPositions, effectiveTicker]);

  const refreshPaperPositions = useCallback(async () => {
    try {
      const [positions, risk] = await Promise.all([
        getOpenPaperPositions(),
        getPaperRiskStatus().catch(() => null),
      ]);
      setOpenPaperPositions(positions);
      setPaperRisk(risk);
    } catch (e) {
  ...
  // 초기 로드 + 10s polling fallback
  useEffect(() => {
    refreshPaperPositions();
    paperPollRef.current = setInterval(refreshPaperPositions, 10_000);
    return () => {
      if (paperPollRef.current) clearInterval(paperPollRef.current);
    };
  }, [refreshPaperPositions]);

  const placePaperTrade = useCallback(async (contracts: number = 1) => {
    if (!canTrade) {
      setPaperOrderError(
        directionAgrees
          ? 'Decision Matrix not GO'
          : `Direction conflict: backend=${backendDirection} vs frontend=${frontendDirection}`,
      );
      return;
    }
    setPlacingPaperOrder(true);
    setPaperOrderError(null);
    try {
      await apiOpenPaperPosition({
        ticker: effectiveTicker,
        frontend_direction: frontendDirection,
        contracts,
      ...
      setPaperOrderError(e instanceof Error ? e.message : 'Failed to place paper trade');
    } finally {
      setPlacingPaperOrder(false);
    }
  }, [canTrade, directionAgrees, backendDirection, frontendDirection,
      effectiveTicker, chartInterval, chartPeriod, refreshPaperPositions]);

  const closePaperTrade = useCallback(async (positionId: string, price?: number) => {
    setPlacingPaperOrder(true);
    setPaperOrderError(null);
    try {
      await apiClosePaperPosition(positionId, price);
      await refreshPaperPositions();
    } catch (e: unknown) {
      setPaperOrderError(e instanceof Error ? e.message : 'Failed to close position');
    } finally {
      setPlacingPaperOrder(false);
    }
  }, [refreshPaperPositions]);
  ...
  const [wfTicker, setWfTicker] = useState<string>('ES=F');
  const [wfIsMicro, setWfIsMicro] = useState(false);
  ...
  const runWalkForward = useCallback(async () => {
    ...
    try {
      // 백그라운드 시작
      const resp = await triggerWalkForward({
        ticker: wfTicker,
        initial_equity: 100_000,
        is_micro: wfIsMicro,
      });
      // 동기 응답에 result가 이미 포함된 경우 (현재 백엔드 구현)
      if (resp.result) {
        setWfResult(resp.result);
        setWfProgress(100);
      }
    } catch (e: unknown) {
  ...
  return {
    ...
    refreshPaperPositions,
    // Walk-Forward
    wfTicker, setWfTicker,
    wfIsMicro, setWfIsMicro,
    wfRunning, wfProgress, wfResult, wfError,
    runWalkForward,
  };
}
