...
export function OptionCalculator() {
  ...
  // iframe 로드 완료 감지 + 콘텐츠 높이 추적 → 페이지 전체 스크롤로 동작
  const syncIframeHeight = useCallback(() => {
    const iframe = iframeRef.current;
    const doc = iframe?.contentDocument;
    if (!iframe || !doc?.body) return;
    // body.scrollHeight 가 콘텐츠의 실제 높이 (same-origin이라 직접 접근 가능)
    const h = Math.max(doc.body.scrollHeight, doc.documentElement.scrollHeight);
    if (h > 0) iframe.style.height = `${h + 24}px`; // 하단 여유분
  }, []);
  ...
  const handleIframeLoad = useCallback(() => {
    ...
    // 콘텐츠 변경(탭 전환, 행 추가 등)에도 추적
    const iframe = iframeRef.current;
    const doc = iframe?.contentDocument;
    if (!doc?.body) return;
    const ro = new ResizeObserver(syncIframeHeight);
    ro.observe(doc.body);
    // cleanup은 컴포넌트 unmount 시 iframe이 사라지면서 함께 해제됨
  }, [syncIframeHeight]);
